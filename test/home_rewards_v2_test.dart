...
void main() {
  testWidgets('Accueil conserve Premium et la consultation gratuite', (
    t,
  ) async {
        ...
        ApiClient(
          httpClient: MockClient(
            (_) async => http.Response(
              jsonEncode({'questions_available': 1, 'progress': 4}),
              200,
    ...
    expect(find.text('1 question gratuite'), findsOneWidget);
    expect(find.text('Ouvrir la consultation →'), findsOneWidget);
  ...
  testWidgets(
    'Accueil explique comment obtenir une question quand le solde est à zéro',
    (t) async {
      final c = RewardsController(
        api: RewardsApi(
          ApiClient(
            httpClient: MockClient(
              (_) async => http.Response(
                jsonEncode({'questions_available': 0, 'progress': 0}),
                200,
                headers: {'content-type': 'application/json'},
              ),
            ),
      ...
      await t.pump(const Duration(seconds: 1));
      expect(find.text('0 question gratuite'), findsOneWidget);
      expect(find.text('Obtenir des questions →'), findsOneWidget);
    },
  );
}
